...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
    ...
    /* stack */
    cirListDeque *known_vertices = malloc(sizeof(cirListDeque));
    Vertex *cur;

    /* set up the queue and load the source vertex */
    initCirListDeque(known_vertices);
    addBackCirListDeque(known_vertices, source);

    /* search until either the destination is found via connections
       or we run out of edges */
    while (!found && !isEmptyCirListDeque(known_vertices)) {
        /* dequeue first node */
        cur = backCirListDeque(known_vertices);
        removeBackCirListDeque(known_vertices);

        /* if the current node is the destination, we're done */
        if (EQ(cur, destination))
            found = 1;
        /* otherwise, if we haven't found a connection to it before
           add its neighbors to the queue */
        else if (!cur->isVisited) {
            cur->isVisited = 1;
            for (i = 0; i < cur->numNeighbors; ++i)
                if (!(cur->neighbors[i]->isVisited))
                    addBackCirListDeque(known_vertices, cur->neighbors[i]);
        }
    }

    /* clean things up */
    clearVisited(g);
    removeAllCirListDeque(known_vertices);
    free(known_vertices);
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
    ...
    /* search until either the destination is found via connections
       or we run out of edges */
    while (!found && !isEmptyCirListDeque(known_vertices)) {
        /* take top node off stack */
        cur = frontCirListDeque(known_vertices);
        removeFrontCirListDeque(known_vertices);
