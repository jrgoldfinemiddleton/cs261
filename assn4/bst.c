...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    curNode = tree->root;
    while (compare(val, curNode->val) != 0) {
        ...
        if (curNode == NULL)
            return 0;       /* dead end, not found */
    }

    return 1;               /* while loop end means found */
}
...
struct Node *_removeLeftMost(struct Node *cur)
{
    ...
    /* base case, move tree rooted at right child up to
     * position of old deleted node */
    if (cur->left == NULL) {
        temp = cur->right;
        cur_data = (struct data *) cur->val;
        free(cur_data);
        free(cur);
        cur = NULL;
        return temp;
