...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
    ...
    pos = sizeDynArr(heap); /* next open position in heap */
    addDynArr(heap, val);

    while (pos != 0) {      /* swap parent with child if needed */
        parent = (pos - 1) / 2;
        if (compare(getDynArr(heap, pos), getDynArr(heap, parent)) < 0) {
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
    ...
    if (rightChild < max) {         /* if node at pos has both children */
        smallChild = _smallerIndexHeap(heap, leftChild, rightChild, compare);

        /* swap node at pos with smallest child if needed */
        if (compare(getDynArr(heap, pos), getDynArr(heap, smallChild)) > 0) {
            swapDynArr(heap, pos, smallChild);
            _adjustHeap(heap, max, smallChild, compare);
        }
    } else if (leftChild < max) {   /* if node at pos has only left child */

        /* swap node at pos with its left child if needed */
...
void removeMinHeap(DynArr *heap, comparator compare)
{
    ...
    /* move the bottom rightmost child up to root and delete that child node */
    putDynArr(heap, 0, getDynArr(heap, last));
    removeAtDynArr(heap, last);
    
    /* no need to adjust heap if there is only a root node */
...
void _buildHeap(DynArr *heap, comparator compare)
{
    ...
    /* for each internal node, adjust the heap */
    while (pos >= 0) {
        _adjustHeap(heap, size, pos, compare);
        --pos;
    }
}
/*
...
void sortHeap(DynArr *heap, comparator compare)
{
  /* FIXME: Write this */
    ...
    last = sizeDynArr(heap) - 1; /* starting index of sorted array */
    assert (last > 0);

    /* make the heap from the unsorted array */
    _buildHeap(heap, compare);

    /* repeatedly take the root and move it to the next position of sorted array */
    while (last > 0) {
